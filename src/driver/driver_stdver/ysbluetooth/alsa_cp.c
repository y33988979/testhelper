...
static int file_type = ALSA_FILETYPE_DEFAULT;
...
int main(int argc, char *argv[])
{
	...
	command = argv[0];
	file_type = ALSA_FILETYPE_DEFAULT;
	if (strstr(argv[0], "arecord")) {
		stream = SND_PCM_STREAM_CAPTURE;
		file_type = ALSA_FILETYPE_WAVE;
		command = "arecord";
		start_delay = 1;
	...
	while ((c = getopt_long(argc, argv, short_options, long_options, &option_index)) != -1) {
		...
		case 'f':
			if (strcasecmp(optarg, "cd") == 0 || strcasecmp(optarg, "cdr") == 0) {
				if (strcasecmp(optarg, "cdr") == 0)
					...
					rhwparams.format = file_type == ALSA_FILETYPE_AU ? SND_PCM_FORMAT_S16_BE : SND_PCM_FORMAT_S16_LE;
		...
		case 'C':
			...
			command = "arecord";
			start_delay = 1;
			if (file_type == ALSA_FILETYPE_DEFAULT)
				file_type = ALSA_FILETYPE_WAVE;
			break;
...
static void rest_global_variables()
{
	readi_func = NULL;
	writei_func = NULL;
	readn_func = NULL;
	...
	vumeter = VUMETER_NONE;
	buffer_pos = 0;
	bits_per_sample = bits_per_frame = 0;
	chunk_bytes = 0;
	test_position = 0;
	test_coef = 8;
	test_nowait = 0;
...
{
		XXLOG();
		rest_global_variables();

		if (!filename)
		{
			LOG0("Filename is invalid!\n");
			return -1;
		}

		char *pcm_name = ALSA_PCMDEV_BT_PLAYBACK;
		int tmp, err, c;
//		int do_device_list = 0, do_pcm_list = 0;
		snd_pcm_info_t *info;
...
int alsa_capture_start(char *filename, alsa_param_t *param)
{
	...
	if (param->channel < 1 || param->channel > 32 )
	{
		LOG0("Channel is invalid! Set to  1 \n");
		param->channel = 1;
	}
	rhwparams.channels = param->channel;
	
	if (param->file_type <= ALSA_FILETYPE_DEFAULT || param->file_type > ALSA_FILETYPE_AU)
	{
		LOG0("Filetype is invalid! Set to WAVE\n");
		param->file_type = ALSA_FILETYPE_WAVE;
	}
	file_type = param->file_type;
	
	if (param->sample_format != ALSA_SAMPLE_FORMAT_CD)
	{
		LOG0("Sample format is invalid! \n");
		return -1;
	}
	else
	{
...
//		rhwparams.channels = 2;
	...
	if (param->sample_rate < 2000 || param->sample_rate > 192000)
	{
		LOG0("Sample rate is invalid! Set to 16000\n");
		param->sample_rate = 16000;
	}
	rhwparams.rate = param->sample_rate;

	char *pcm_name = ALSA_PCMDEV_BT_CAPTURE;
	int tmp, err, c;
	snd_pcm_info_t *info;
...
static void set_params(void)
{
	...
	printf ("(1)hwparams.format = %d\n", hwparams.format);
	snd_pcm_sw_params_alloca(&swparams);
	err = snd_pcm_hw_params_any(handle, params);
	printf ("(2)hwparams.format = %d\n", hwparams.format);
	if (err < 0) {
...
#if 1
	rate = hwparams.rate;
	LOG1("Before hwparams.rate=%d\n", hwparams.rate);
	err = snd_pcm_hw_params_set_rate_near(handle, params, &hwparams.rate, 0);
	LOG1("After hwparams.rate=%d\n", hwparams.rate);
	assert(err >= 0);
...
#else
	LOG1("Before hwparams.rate=%d\n", hwparams.rate);
	err = snd_pcm_hw_params_set_rate_resample(handle, params, hwparams.rate);
	LOG1("After hwparams.rate=%d\n", hwparams.rate);
#endif
...
//			printf ("%s | >err  playback (%s)\n", __FUNCTION__, name);
	...
	if (test_au(fd, audiobuf) >= 0) {
		...
		playback_go(fd, 0, pbrec_count, ALSA_FILETYPE_AU, name);
		goto __end;
	}
	...
	if ((dtawave = test_wavefile(fd, audiobuf, dta)) >= 0) {
		pbrec_count = calc_count();
		playback_go(fd, dtawave, pbrec_count, ALSA_FILETYPE_WAVE, name);
	} else {
		/* should be raw data */
		init_raw_data();
		pbrec_count = calc_count();
		playback_go(fd, dta, pbrec_count, ALSA_FILETYPE_RAW, name);
	}
      __end:
	if (fd != 0)
		close(fd);
...
static void capture(char *orig_name)
{
	...
	do {
		 ...
		 */
	} while ((file_type == ALSA_FILETYPE_RAW && !timelimit) || count > 0);
}
...
static void playbackv(char **names, unsigned int count)
{
	...
	init_raw_data();
	pbrec_count = calc_count();
	playbackv_go(fds, channels, 0, pbrec_count, ALSA_FILETYPE_RAW, names);
...
static void capturev(char **names, unsigned int count)
{
	...
	init_raw_data();
	pbrec_count = calc_count();
	capturev_go(fds, channels, pbrec_count, ALSA_FILETYPE_RAW, names);
