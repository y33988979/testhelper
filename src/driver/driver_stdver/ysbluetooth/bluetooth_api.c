...
int get_adapter_bdaddr(const char * hci_name,bdaddr_t *bdaddr)
{
	...
	if(id < 0 || id > HCI_MAX_DEV){
		printf("invalid hci name,use default\n");
		id = hci_get_route(NULL);
	}
	
	if ((ctl = socket(AF_BLUETOOTH, SOCK_RAW, BTPROTO_HCI)) < 0) {
		printf("Can't open HCI socket.check kernel network config\n");
		return -1;
	}
...
int enable_adapter(const char* hci_name, int enable)
{
    ...
    if(result < 0) {
		printf("Can't init device %s %s (%d)\n",hci_name ? hci_name: "hci0",
...
{ 
    ...
    if( id < 0 || id > HCI_MAX_DEV ){	
    	printf("invalid hci name:%s,use default\n",hci_name);
    	id = hci_get_route(NULL);
    }
    ...
    ii = (inquiry_info*)malloc(max * sizeof(inquiry_info));
    if(!ii){
    	printf("malloc inquiry_info failed\n");
    	result = -1;
    	goto end1;
    }
    ...
    if( num_rsp < 0 ){ 
    	printf("hci_inquiry failed,id=%d,len=%d,result=%d\n",id,len,num_rsp);
    	result = -1;
    	goto end2;
    }
						...
						DBusMessage *msg, void *data)
{
	...
	if (!dbus_message_get_args(msg, NULL, DBUS_TYPE_OBJECT_PATH, &path,
							DBUS_TYPE_INVALID)) {
		printf( "Invalid arguments for RequestPinCode method");
		return DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
	}
	...
	if (!reply) {
		printf( "Can't create reply message\n");
		return DBUS_HANDLER_RESULT_NEED_MEMORY;
	}

	printf("Pincode request for device %s,passkey: %s\n", path,passkey_value);
	/*
	if (passkey_delay) {
						...
						DBusMessage *msg, void *data)
{
	...
	if (!dbus_message_get_args(msg, NULL, DBUS_TYPE_OBJECT_PATH, &path,
							DBUS_TYPE_INVALID)) {
		printf( "Invalid arguments for RequestPasskey method");
		return DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
	}
						...
						DBusMessage *msg, void *data)
{
	...
	if (!dbus_message_get_args(msg, NULL, DBUS_TYPE_OBJECT_PATH, &path,
						DBUS_TYPE_STRING, &uuid,
							DBUS_TYPE_INVALID)) {
		printf( "Invalid arguments for Authorize method");
		return DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
	}
						...
						DBusMessage *msg, void *data)
{
	...
	if (!dbus_message_get_args(msg, NULL, DBUS_TYPE_INVALID)) {
		printf( "Invalid arguments for passkey Confirm method");
		return DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
	}
						...
						DBusMessage *msg, void *data)
{
	...
	if (!dbus_message_get_args(msg, NULL, DBUS_TYPE_INVALID)) {
		printf( "Invalid arguments for Release method");
		return DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
	}

	if (!__io_canceled)
		printf( "Agent has been released\n");
						...
						const char *device)
{
	...
	if (!success) {
		printf( "Not enough memory for message send\n");
		return -1;
	}
...
static char *get_adapter_path_api(DBusConnection *conn, const char *adapter)
{
	DBusMessage *msg, *reply;
	DBusError err;
	const char *reply_path;
...
#define PARI_RETRIES	100 	
	...
	if (!conn) {
		printf("Can't get on system bus");
		return -1;
	}
	
	adapter_path = get_adapter_path_api(conn, hci_name);
	if (!adapter_path){
	...
	printf("adapter path %s,agent_path:%s,passkey:%s\n",
	...
	if (!dbus_connection_try_register_object_path(conn, agent_path,
		...
		printf( "Can't register object path for agent\n");
	...
	*/
	...
	if (create_paired_device(conn, adapter_path, agent_path,
		...
		result = -1;
		printf("paired with device failed\n");
		goto end3;
	}

	if (!dbus_connection_add_filter(conn, agent_filter, NULL, NULL))
		printf( "Can't add signal filter");
...
static int parse_dbus_msg_uint(DBusMessageIter *iter)
{
	...
	if( (type != DBUS_TYPE_UINT32) && (type != DBUS_TYPE_UINT16)
		...
		printf("invalid dbus type,type=%c\n",(char)type);
		return -1;
	}
...
static void* get_dbus_msg_subiter(DBusMessageIter *iter)
{
	...
	if( (type != DBUS_TYPE_ARRAY) && (type != DBUS_TYPE_VARIANT) 
		...
		printf("invalied dbus type,type=%c\n",(char)type);
		return NULL;
	}	
	
	psubiter =(DBusMessageIter*)malloc(sizeof(DBusMessageIter));
	if(!psubiter)
		return NULL;
...
static int get_device_properties(
	...
	while (type == DBUS_TYPE_DICT_ENTRY) {
		...
		if(!strcmp(tmp,property)){
			...
			type = dbus_message_iter_get_arg_type(subsubiter);
			if(p_type != type){
				printf("invalid properties type,desire_type:%c,actual_type:%c\n",
			...
			case DBUS_TYPE_UINT16:
				result = parse_dbus_msg_uint(subsubiter);
				if(result < 0){
					printf("parse uint failed,result:%d\n",result);
					result = -1;
					break;
				}	
			...
			default:
				printf("%s:invalid type:%d",__func__,type);
				result = -1;
				break;
			}	
			release_subiter(subsubiter);	
...
{	
	...
	result = get_device_properties("org.bluez.Headset",conn,dev_path,"Connected",DBUS_TYPE_BOOLEAN,status);
...
{	
	...
	for (i = 0; i < cl->conn_num; i++, ci++) {
	    ...
	    ret = get_device_properties("org.bluez.Headset",conn,dev_path,"Connected",DBUS_TYPE_BOOLEAN,&status);
        if((ret == 0)&&(status==1))
        {
...
//        dbus_bus_add_match(conn, "type=signal,interface=org.bluez.Headset", NULL);
	...
	if(connect){
        ...
        if(result != 0){
		    printf("failed to get headset connect status\n");
            goto finished;
        }else if(status == 1){
            goto finished;
        }
        ...
        if(result != 0){
		    printf("failed to get device connect status\n");
            goto finished;
        }else if(status == 1){
//			printf("socket alreay connect,should disconnect,getuid %d\r\n",getuid());
	...
	if(!reply){
		if (dbus_error_is_set(&error)) {
			printf( "connect error message: %s\n", error.message);
			dbus_error_free(&error);
		} else
			printf( "Failed to connect to device\n");	
        result = -1;
