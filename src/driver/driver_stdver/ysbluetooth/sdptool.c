...
#if 0
static int sdp_rfcomm_comp(void *value, void *user)
{
...
static int sdp_rfcomm_comp(void *value_list, void *user)
{
	
	sdp_list_t *list = (sdp_list_t *)value_list;
	for (; list; list = list->next)
	{
		void *value = list->data;
		char str[MAX_LEN_PROTOCOL_UUID_STR];
		sdp_data_t *p = (sdp_data_t *)value, *s;
		int i = 0, proto = 0;
...
static int get_access_protos(void *value, void *userData, sdp_comp_func_t f)
{
	sdp_list_t *protDescSeq = (sdp_list_t *)value;
//	return sdp_list_foreach(protDescSeq, f, 0);
	sdp_list_foreach(protDescSeq, f, 0);

}


int get_oop_channel(char *dest_addr, char *service_name)
{
	struct search_context context;
	...
	for (; seq; seq = next)
	{
		...
		/* Display user friendly form */
		sdp_data_t *sd = sdp_data_get(rec, SDP_ATTR_SVCNAME_PRIMARY);
		if (sd)
		{
			printf("Service Name: %.*s\n", sd->unitSize, sd->val.str);
			if (strcmp(service_name, sd->val.str))
			{	
				next = seq->next;
				free(seq);
				sdp_record_free(rec);
		...
		LOG0 ("Found OBEX Object Push Service!!\n");

		sdp_list_t *proto = 0, *result = 0;
		
		if (sdp_get_access_protos(rec, &proto) == 0) 
		{
			LOG1("Protocol Descriptor List:\n");
			result = sdp_list_find(proto, &channel, sdp_rfcomm_comp);
			sdp_list_foreach(proto, (sdp_list_func_t)sdp_list_free, 0);
			sdp_list_free(proto, 0);
		}
