		  ...
		  iwrange *	range)
{
  ...
  if(!iw_ignore_version)
	...
	{
	  printf("Warning: Driver for device %s has been compiled with an ancient version\n", ifname);
	  printf("of Wireless Extension, while this program support version 11 and later.\n");
	  printf("Some things may be broken...\n\n");
	}
	...
	{
	  printf("Warning: Driver for device %s has been compiled with version %d\n", ifname, range->we_version_compiled);
	  printf("of Wireless Extension, while this program supports up to version %d.\n", WE_MAX_VERSION);
	  printf("Some things may be broken...\n\n");
	}
	...
	{
	  printf("Warning: Driver for device %s recommend version %d of Wireless Extension,\n", ifname, range->we_version_source);
	  printf("but has been compiled with version %d, therefore some driver features\n", range->we_version_compiled);
	  printf("may not be available...\n\n");
	}
 ...
 * Read /proc/net/wireless to get the latest statistics
 */
int iw_getstats(char *	ifname, iwstats *stats)
{
	       ...
	       int		has_range)
{
  ...
  if(has_range && ((qual->level != 0)
		   || (qual->updated & (IW_QUAL_DBM /*| IW_QUAL_RCPI for we>20*/))))
    {
      ...
      if(!(qual->updated & IW_QUAL_QUAL_INVALID))
	{
      ...
      if(0/*qual->updated & IW_QUAL_RCPI for we>20 */)
	{
                    			...
                    			int	we_version)
{
  ...
  printf("DBG - stream->current = %p, stream->value = %p, stream->end = %p\n",
    ...
    * Note : the event may be unaligned, therefore copy... */
    /*
    * here get cmd and len total 4 bytes
    * mem copy can avoid unaligned data ????
    */
  ...
  printf("DBG - event_type = %d, event_len = %d, pointer = %p\n",
    ...
    else
    {
        ...
        * Fixing that in the kernel would break 64 bits userspace. */
        if((stream->value == NULL)
        && ((((iwe->len - IW_EV_LCP_LEN) % event_len) == 4)
