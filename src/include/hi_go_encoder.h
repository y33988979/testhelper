/**
 \file
 \brief Describes the header file of the encoder module. CNcomment:encoder头文件
...
/** 
\brief Deinitializes the encoder. CNcomment:编码器去初始化
\attention \n
When ::HI_GO_Deinit is called, this API is also called.
CNcomment: ::HI_GO_Deinit中已包含对该接口的调用
\param  N/A
...
CNcomment:将一个surface编码到指定图像格式并保存到文件中
\attention \n
...
\param[in] hSurface  Surface to be encoded. CNcomment:需要编码的surface,
\param[in] pFile    Name of the encoded file. This parameter can be empty. If this parameter is not set, the encoded file is named [year]-[month]-[date]-[hour]-[minute]-[second].  
                        CNcomment:编码后的文件名，可以为空，为空则用当前时间[年]-[月]-[日]-[时]-[分]-[秒]来命名
\param[in] pAttr    Encoding attributes. This parameter cannot be empty. CNcomment:编码设置的属性，不可为空
...
CNcomment:将一个surface编码到指定图像格式并保存到内存中，请自行保证指定内存块足够大
\attention \n
...
\param[in] hSurface     Surface to be encoded. CNcomment:需要编码的surface,
\param[in] pMem         Start address of the memory for storing the encoded pictures. CNcomment:保存编码后的图像数据内存块的起始地址
\param[in] MemLen       Size of a specified memory. CNcomment:指定内存的大小
\param[out] pOutLen     Actual size of the used memory, indicating the length of encoded data. Ensure that the value of pOutLen is smaller than or equal to the value of MemLen. 
                                CNcomment:实际使用的内存大小，编码后数据长度(自行确保pOutLen <= MemLen)
\param[in] pAttr        Encoding attributes. This parameter cannot be empty. CNcomment:编码设置的属性，不可为空
