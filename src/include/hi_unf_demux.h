...
/**
 * \file
 * \brief Describes the information about the DEMUX module.CNcomment:提供DEMUX的相关信息
 */
...
{
    HI_UNF_DMX_PORT_0_TUNER,     /**<Port0, tuner mode*/ /**< CNcomment:port0, Tuner模式 */
    ...
    HI_UNF_DMX_PORT_3_RAM,       /**<Port3, RAM mode*/ /**< CNcomment:port3, RAM模式 */
...
typedef enum hiUNF_DMX_PORT_TYPE_E
{
    ...
    HI_UNF_DMX_PORT_TYPE_SERIAL,                  /**<Serial mode*/ /**< CNcomment:串行模式                  */
    HI_UNF_DMX_PORT_TYPE_USER_DEFINED,            /**<User defined mode,just for mv300 ip mode*//**< CNcomment:用户自定义模式，只对mv300 IP端口起作用 */
...
{
    HI_UNF_DMX_PORT_MODE_E enPortMod;  /**<Port mode*/ /**< CNcomment:端口模式 */
    ...
    HI_U32 u32UserDefLen1;            /**<User defined length1,valid when enPortType is HI_UNF_DMX_PORT_TYPE_USER_DEFINED,188~255 *//**< CNcomment:用户自定义长度检测包长1，只有端口模式为用户自定义时有效，有效范围188~255 */
    HI_U32 u32UserDefLen2;            /**<User defined length2,valid when enPortType is HI_UNF_DMX_PORT_TYPE_USER_DEFINED,188~255 *//**< CNcomment:用户自定义长度检测包长2，只有端口模式为用户自定义时有效，有效范围188~255*/
...
/**
\brief Initializes the DEMUX module.CNcomment:初始化DEMUX模块
\attention \n
...
CNcomment:查询DEMUX模块业务能力。业务功能包括通用通道数目、过滤器数目、密钥数目、端口数目、DEMUX设备数目和音视频通道数目等
\attention \n
N/A.CNcomment:无
\param[out] pstCap   Pointer to the capability of the DEMUX module (output).CNcomment:指针类型，输出DEMUX模块能力。
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
...
\attention \n
 N/A.CNcomment:无
\param[in] enPortId  u32PortId  Port ID.CNcomment:端口号
\param[out] pstAttr   Pointer to the port attributes (output).CNcomment:指针类型，输出端口属性。
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
...
\param[in] enPortId    Port ID.CNcomment:端口号。
\param[in] pstAttr    Pointer to port attributes.CNcomment:指针类型，端口属性
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails. CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
...
CNcomment:支持5路DEMUX，每路DEMUX只能绑定到一个PORT，但是一个PORT的数据可以分发到多个DEMUX\n
...
\attention \n
If a DEMUX is not attached to any port, an error code is returned.CNcomment:如果DEMUX没有绑定到任何端口上会返回错误码
\param[in] u32DmxId  DEMUX ID.
\param[out] penPortId  Pointer to the ID of the port that is attached to a DEMUX(output).CNcomment:指针类型，输出DEMUX绑定的端口号。
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
\retval ::HI_ERR_DMX_NULL_PTR  The pointer is null. CNcomment:指针参数为空
\retval ::HI_ERR_DMX_NOATTACH_PORT  The DEMUX is not attached to any port.CNcomment:此路DEMUX没有绑定到任何端口上
\see \n
 N/A.CNcomment:无
*/
...
/**
\brief Creates a TS buffer for the RAM port to receive the TS data from Internet or local memory. CNcomment:为RAM端口创建TS Buffer用于接收从网络或本地存储中输入的TS数据
\attention \n
...
CNcomment:只有RAM类型端口可以创建TS Buffer\n
buffer大小必须在4K和16M之间，单位为字节\n
buffer大小最好为4K的整数倍，如果不是整数倍，内部会进行向下对齐操作\n
同一个端口不能重复创建TS Buffer,否则返回错误码
\param[in] enPortId  [in]  Port ID.CNcomment:端口号。
\param[in] u32TsBufSize Size of a TS buffer.CNcomment:TS Buffer大小
\param[out] phTsBuffer  Pointer to the handle of a created TS buffer (output).CNcomment:指针类型，输出创建的TS Buffer 句柄。
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
\retval ::HI_ERR_DMX_NULL_PTR  The pointer is null. CNcomment:指针参数为空
\retval ::HI_ERR_DMX_RECREAT_TSBUFFER  The TS buffer corresponding to the port is created.CNcomment:重复创建TS Buffer
\see \n
N/A.CNcomment:无
*/
...
/**
\brief Obtains a TS buffer to input data.CNcomment:获取TS Buffer空间，用于数据输入
\attention \n
...
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
\retval ::HI_ERR_DMX_NULL_PTR  The pointer is null. CNcomment:指针参数为空
\retval ::HI_ERR_DMX_TIMEOUT  The wait times out. CNcomment:等待超时
\retval ::HI_ERR_DMX_NOAVAILABLE_BUF  The size of the available buffer is smaller than the requested data length.CNcomment:剩余buffer空间小于请求长度
\see \n
 N/A.CNcomment:无
*/
...
/**
\brief Updates the write pointer of a TS buffer after the TS data is input.CNcomment:TS数据输入完毕，用于更新TS Buffer写指针
\attention \n
...
\param[in] hTsBuffer Handle of a TS buffer.CNcomment:TS Buffer句柄
\param[in] u32ValidDataLen Valid data length.CNcomment:有效数据的长度
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
...
/**
\brief Resets a TS buffer to clear its data.CNcomment:复位TS Buffer，清空TS Buffer中的数据
\attention \n
If a TS buffer is reset, you can call HI_UNF_DMX_PutTSBuffer only after calling HI_UNF_DMX_GetTSBuffer.CNcomment:TS Buffer复位后需要重新get才能执行put操作
\param[in] hTsBuffer Handle of a TS buffer.CNcomment:TS Buffer句柄
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
...
\attention \n
You can query the size of a TS buffer and the used buffer size by calling this API. CNcomment:可以查询TS Buffer大小和已用的空间大小
\param[in] hTsBuffer Handle of a TS buffer.CNcomment:TS Buffer句柄
\param[out] pStatus  TS buffer status.CNcomment:TS Buffer状态结构
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
...
\param[in] hTsBuffer Handle of a TS buffer.CNcomment:TS Buffer句柄
\param[out] penPortId  ID of the port.corresponding to a TS buffer  CNcomment:TS Buffer对应的Port ID
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
...
\attention \n
If you obtain the handle of a TS buffer that is not created, an error code is returned.CNcomment:对于没有申请的TS buffer,查询会返回错误
\param[in] enPortId   Port ID
\param[out] phTsBuffer Handle of a TS buffer.CNcomment:TS Buffer句柄
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
...
\param[out] sPortStat   Number of TS packets .CNcomment:TS包计数信息
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
...
\param[out] pstChAttr Pointer to the structure of the channel attributes.CNcomment:通道属性结构体指针
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
\retval ::HI_ERR_DMX_NULL_PTR  The pointer is null. CNcomment:指针参数为空
...
/**
\brief Creates a PID channel based on channel attributes.CNcomment:根据通道属性创建一个PID通道
\attention \n
...
\param[in] u32DmxId  DEMUX ID
\param[in] pstChAttr  Pointer to the attributes of an input channel. For details, see the description of HI_UNF_DMX_CHAN_ATTR_S.CNcomment:指针类型，输入通道属性。请参见::HI_UNF_DMX_CHAN_ATTR_S
\param[out] phChannel  Pointer to the handle of an allocated channel.CNcomment:指针类型，输出分配的通道Handle。
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
\retval ::HI_ERR_DMX_NULL_PTR  The pointer is null. CNcomment:指针参数为空
\retval ::HI_ERR_DMX_NOFREE_CHAN  There is no available channel.CNcomment:没有空闲通道
\see \n
 N/A.CNcomment:无
*/
...
/**
\brief Destroys an existing PID channel.CNcomment:销毁创建的PID通道
\attention \n
...
\param[in] hChannel  Channel handle.CNcomment:通道句柄。
\retval none
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
...
/**
\brief Obtains the attributes of a channel.CNcomment:获取通道属性
\attention \n
The API can be called only after a channel is allocated.CNcomment:本接口必须在通道分配以后才可以调用
\param[in] hChannel   Channel handle.CNcomment:通道Handle。
\param[out] pstChAttr  Pointer to the attributes of a specified channel. For details, see the description of HI_UNF_DMX_CHAN_ATTR_S.CNcomment:指针类型，输出指定通道属性。请参见::HI_UNF_DMX_CHAN_ATTR_S
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid.CNcomment:输入参数非法
...
/**
\brief Sets the attributes of a channel.CNcomment:设置通道属性
\attention \n
...
/**
\brief Sets the PID of a channel.CNcomment:设置通道PID
\attention \n
...
\param[in] hChannel   Channel handle.CNcomment:通道句柄。
\param[in] u32Pid     Channel PID.CNcomment:通道PID
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
\retval ::HI_ERR_DMX_OPENING_CHAN  The channel is enabled.CNcomment:通道处于打开状态
\see \n
 N/A.CNcomment:无
*/
...
/**
\brief Obtains the PID of a channel.CNcomment:获取通道配置的PID
\attention \n
...
\param[in] hChannel  Channel handle.CNcomment:通道句柄。
\param[out] pu32Pid   Pointer to the PID of a channel (output).CNcomment:指针类型，输出通道的PID
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
...
/**
\brief Enables a channel.CNcomment:打开通道
\attention \n
...
/**
\brief Disables a channel.CNcomment:关闭通道
\attention \n
If you this API to disable a PID channel that has been disabled, no error occurs.CNcomment:在PID通道关闭状态下，调用本接口关闭通道不会发生错误
\param[in] hChannel   Channel handle.CNcomment:通道句柄。
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
...
/**
\brief Obtains the status of a channel.CNcomment:获取通道状态
\attention \n
You can call this API after a channel is created.CNcomment:可以在通道申请后的任意时刻调用
\param[in] hChannel   Channel handle.CNcomment:通道句柄。
\param[out] pstStatus  Pointer to the status of a channel (output). For details, see the description of ::HI_UNF_DMX_CHAN_STATUS_S.CNcomment:指针类型，输出通道状态。请参见::HI_UNF_DMX_CHAN_STATUS_S
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
...
/**
\brief Obtains the ts count of a channel.CNcomment:获取通道ts包计数
\attention \n
You can call this API after a channel is created.CNcomment:可以在通道申请后的任意时刻调用
\param[in] hChannel   Channel handle.CNcomment:通道句柄。
\param[out] pu32TsCount  Pointer to the count of a channel (output).CNcomment:指针类型，输出通道ts包计数。
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
...
/**
\brief Obtains the handle of the channel with a specific PID.CNcomment:获取配置某PID的通道句柄
\attention \n
...
\param[in] u32DmxId    DEMUX ID
\param[in] u32Pid      Channel PID.CNcomment:通道PID
\param[out] phChannel Pointer to channel handle.CNcomment: 指针类型，输出通道Handle。
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
\retval ::HI_ERR_DMX_NULL_PTR  The pointer is null. CNcomment:指针参数为空
\retval ::HI_ERR_DMX_UNMATCH_CHAN  No matched channel is found.CNcomment:没有查询到匹配通道
\see \n
 N/A.CNcomment:无
*/
...
\param[in] u32DmxId         DEMUX ID。
\param[out] pu32FreeCount   Pointer to the number of available channels of a DEMUX.CNcomment:指针类型，输出DEMUX空闲通道数目
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
...
\param[in] hChannel   Channel handle.CNcomment:通道句柄。
\param[out] penScrambleFlag   Pointer to the scrambled flag of the channel data (output). For details, see the description of ::HI_UNF_DMX_SCRAMBLED_FLAG_E.CNcomment:指针类型，输出通道数据加扰标志。请参见::HI_UNF_DMX_SCRAMBLED_FLAG_E
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
...
\param[in] u32DmxId    DEMUX ID。
\param[in] pstFilterAttr   Pointer to the attributes of a filter (input).CNcomment:指针类型，输入过滤器属性。
\param[out] phFilter   Pointer to the handle of the created filter (output).CNcomment:指针类型，输出申请的过滤器Handle。
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
\retval ::HI_ERR_DMX_NULL_PTR  The pointer is null. CNcomment:指针参数为空
\retval ::HI_ERR_DMX_NOFREE_FILTER There is no available filter.CNcomment:没有空闲filter
\see \n
 N/A.CNcomment:无
*/
...
\param[in] hFilter  Filter handle.CNcomment:过滤器句柄。
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
...
\attention \n
You can call this API to release all filters attached to a channel when the channel is enabled or disabled.CNcomment:允许在通道打开或者关闭状态下，调用本接口释放通道上绑定的所有过滤器
\param[in] hChannel  Channel handle.CNcomment:通道句柄。
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is valid.CNcomment:输入参数非法
\see \n
N/A.CNcomment:无
*/
...
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
\retval ::HI_ERR_DMX_NULL_PTR  The pointer is null. CNcomment:指针参数为空
\retval ::HI_ERR_DMX_ATTACHED_FILTER  The attributes cannot be set dynamically, because the filter is attached to a channel.CNcomment:过滤器绑定在通道上，不能动态设置
\see \n
 N/A.CNcomment:无
*/
...
\param[in] hFilter   Filter handle.CNcomment:过滤器句柄。
\param[out] pstFilterAttr   Pointer to the attributes of a filter (output). For details, see the description of ::HI_UNF_DMX_FILTER_ATTR_S.CNcomment:指针类型，输出过滤器属性。请参见::HI_UNF_DMX_FILTER_ATTR_S
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
...
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
\retval ::HI_ERR_DMX_ATTACHED_FILTER  The filter is attached.CNcomment:过滤器已经处于绑定状态
\retval ::HI_ERR_DMX_NOT_SUPPORT   No filter can be attached to the channel.CNcomment:此类型通道不支持绑定过滤器
\see \n
 N/A.CNcomment:无
*/
...
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
\retval ::HI_ERR_DMX_NOATTACH_FILTER  No filter is attached to any channel.CNcomment:filter没有绑定在任何通道上
\retval ::HI_ERR_DMX_UNMATCH_FILTER  No filter is attached to the channel.CNcomment:filter没有绑定在此通道上
\see \n
 N/A.CNcomment:无
*/
...
\attention \n
Each filter corresponds to a channel. If a filter is not attached to a channel, the error code HI_ERR_DMX_NOATTACH_FILTER is returned.CNcomment:每个过滤器唯一对应一个通道。若过滤器没有绑定任何到通道上，则返回HI_ERR_DMX_NOATTACH_FILTER错误码
\param[in] hFilter    Filter handle.CNcomment:过滤器句柄。
\param[out] phChannel   Pointer to the handle of the channel to which a filter is attached (output).CNcomment:指针类型，输出过滤器绑定的通道句柄
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
...
\param[in] u32DmxId   DEMUX ID。
\param[out]  pu32FreeCount  Pointer to the number of allocated filters (output).CNcomment:指针类型，输出当前未分配的过滤器数目
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
...
/**
\brief Creates a key area.CNcomment:创建一个密钥区
\attention \n
...
\param[in] u32DmxId   DEMUX ID。
\param[out] phKey     Pointer to the handle of a created key area.CNcomment:指针类型，输出申请到的密钥区Handle。
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
\retval ::HI_ERR_DMX_NULL_PTR  The pointer is null. CNcomment:指针参数为空
\retval ::HI_ERR_DMX_NOFREE_KEY There is no available key area. CNcomment:没有空闲的密钥区
\see \n
 N/A.CNcomment:无
*/
...
/**
\brief Creates a key area. The key area type and descrambling protocol type can be selected.CNcomment:创建一个密钥区,支持选择高安全CA和解扰协议类型
\attention \n
...
\param[in] u32DmxId   DEMUX ID
\param[in] pstDesramblerAttr  Pointer to the attributes of a key area.CNcomment:密钥区属性指针  
\param[out] phKey      Pointer to the handle of a created key area.CNcomment:指针类型，输出申请到的密钥区Handle。
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
...
\retval ::HI_ERR_DMX_NULL_PTR  The pointer is null. CNcomment:指针参数为空
\retval ::HI_ERR_DMX_NOFREE_KEY  There is no available key area.CNcomment:没有空闲的密钥区
\retval ::HI_ERR_DMX_NOT_SUPPORT  Not support HI_UNF_DMX_DESCRAMBLER_ATTR_S type.CNcomment:不支持的HI_UNF_DMX_DESCRAMBLER_ATTR_S类型
\see \n
 N/A.CNcomment:无
*/
...
\param[in] hKey  Handle of the key area to be destroyed.CNcomment:待删除的密钥区Handle。
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
...
/**
\brief Sets the even keys of a key area. This API is used to configure the DEMUX descrambler based on even keys after the CA system obtains control words.CNcomment:设置密钥区的偶密钥。CA系统得到控制字后，可调用本接口将偶密钥配置到DEMUX解扰模块
\attention \n
...
\param[in] hKey  Handle of the key area to be set.CNcomment:待设置的密钥区句柄。
\param[in] pu8EvenKey  Pointer to the 16-byte even key data to be set.CNcomment:指针类型，指向要设置的偶密钥数据，必须是16个字节的数组
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
...
/**
\brief Sets the odd keys of a key area. This API is used to configure the DEMUX descrambler based on odd keys after the CA system obtains control words.CNcomment:设置密钥区的奇密钥。CA系统得到控制字后，可调用本接口将奇密钥配置到DEMUX解扰模块
\attention \n
...
当设置密钥之前，密钥的初时值都是0，表示不做解扰
\param[in] hKey  Handle of the key area to be set.CNcomment:待设置的密钥区句柄。
\param[in] pu8OddKey   Pointer to the 16-byte odd key data to be set.CNcomment:指针类型，指向要设置的奇密钥数据，必须是16个字节的数组
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
...
/**
\brief Set Even IV.For algs do not use IV,do not care.CNcomment:设置偶密钥区的初始化向量。对于不涉及初始化向量的解扰算法可以不关注。
\attention \n
...
\param[in] hKey  Handle of the key area to be set.CNcomment:待设置的密钥区句柄。
\param[in] pu8IVKey   Pointer to the 16-byte IV key data to be set.CNcomment:指针类型，指向要设置的奇密钥数据，必须是16个字节的数组
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
...
/**
\brief Set Odd IV.For algs do not use IV,do not care.CNcomment:设置奇密钥区的初始化向量。对于不涉及初始化向量的解扰算法可以不关注。
\attention \n
pu8IVKey points to the iv key data to be set.The data consists of 16 bytes: CW1, CW2, ?-, and CW16.\n
The key value can be set dynamically, that is, the key value can be set at any time after a key area is created.
CNcomment:pu8IVKey指向要设置的初始化向量数据。奇密钥数据共16byte，byte依次是CW1、CW2、……、CW16\n
...
CNcomment:一个密钥区可以绑定到多个通道上，通道可以属于不路的DEMUX\n
可以对所有类型的通道绑定密钥区进行静荷数据的解扰\n
不允许重复绑定相同或不同的密钥区到同一个通道上
\param[in] hKey    Handle of the key area to be attached.CNcomment:待绑定的密钥区句柄。
\param[in] hChannel   Channel handle.CNcomment:通道句柄。
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
...
\param[in] hKey    Handle of the key area to be detached.CNcomment:待解绑定的密钥区句柄。
\param[in] hChannel  Channel handle.CNcomment:通道句柄。
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
\retval ::HI_ERR_DMX_NOATTACH_KEY  No key areas are attached to the channel.CNcomment:通道上没有绑定任何密钥区
\retval ::HI_ERR_DMX_UNMATCH_KEY  The specified key area is not attached to the specified channel.CNcomment:指定的密钥区没有绑定在指定的通道上
\see \n
 N/A.CNcomment:无
*/
...
CNcomment:当通道没有绑定密钥区时，调用本接口返回HI_ERR_DMX_NOATTACH_KEY错误码
\param[in] hChannel  Handle of the channel to be queried.CNcomment:要查询的通道句柄。
\param[out] phKey     Pointer to the handle of the key area that is attached to a channel (output).CNcomment:指针类型，输出通道绑定的密钥区句柄
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
...
\attention \n
Because key areas are shared by multiple DEMUXs, the first parameter is meaningless. Therefore, you only need to enter a valid value.CNcomment:密钥区在几路DEMUX之间共用，所以第一个参数没有意义，只要输入一个合法值就可以了。
\param[in] u32DmxId   DEMUX ID
\param[out]  pu32FreeCount   Pointer to the number of available key areas (output).CNcomment:指针类型，输出空闲密钥区数目
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
...
/**
\brief  Obtains the number of available key areas, including advanced CA key areas and common CA key areas.CNcomment:获取空闲密钥区数量，包含高安全CA和普通CA
\attention \n
This API can be used to query the numbers of advanced CA key areas and common CA key areas.
CNcomment:可以最为高安全CA和普通CA个数查询的接口使用。
\param[out]  pu32FreeNormal  Pointer to the number of available common CA key areas.CNcomment:指针类型，输出通用CA空闲密钥区数目
\param[out]  pu32FreeAdvance   Pointer to the number of available advanced CA key areas.CNcomment:指针类型，输出高安全CA空闲密钥区数目
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_NULL_PTR  The pointer is null. CNcomment:指针参数为空
...
\param[out] phChannel      Pointer to the number of channels. The number of channels depends on the value of pu32ChNum.CNcomment:指针类型，指向通道句柄数组。数组长度有pu32ChNum决定
...
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
\retval ::HI_ERR_DMX_NULL_PTR  The pointer is null. CNcomment:指针参数为空
\retval ::HI_ERR_DMX_NOAVAILABLE_DATA  No data is received in any channel.CNcomment:没有任何通道有数据到达
\retval ::HI_ERR_DMX_TIMEOUT  The wait times out.CNcomment:等待超时
\see \n
 N/A.CNcomment:无
*/
...
\param[in] phWatchChannel   Pointer to the number of handles of the channels to be queried. The number depends on the value of u32WatchNum.CNcomment:指针类型，指向被监视通道的句柄数组。数组长度有u32WatchNum决定
\param[in] u32WatchNum      Number of channels to be queried.CNcomment:监视通道的个数
\param[in] phDataChannel    Pointer to the number of handles of the channels that contain data The number depends on the value of pu32ChNum.CNcomment:指针类型，指向存在数据的通道的句柄数组。数组长度有pu32ChNum决定
\param[out] pu32ChNum       Pointer to the number of lower-layer channels that receive new data. The maximum value cannot be greater than the input value.CNcomment:指针类型。底层有新数据通道的数目；最多不超过被监视的通道的数值
\param[in] u32TimeOutMs     Wait timeout, in ms.CNcomment:等待超时时间，单位ms
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
...
\param[in] hChannel            Channel handle.CNcomment:通道句柄。
\param[in] u32AcquireNum       Maximum number of obtained data packets.CNcomment:请求的数据包的最大个数
\param[out] pu32AcquiredNum    Pointer to the actual number of obtained data packets.CNcomment:指针类型，输出实际请求到的数据包个数
\param[out] pstBuf             Pointer to the number of data packets. The number depends on the value of pu32AcquiredNum.CNcomment:指针类型，指向数据包信息数组，数组个数由pu32AcquiredNum决定
\param[in] u32TimeOutMs        Wait timeout, in ms.CNcomment:等待超时时间，单位为ms
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
\retval ::HI_ERR_DMX_NULL_PTR  The pointer is null. CNcomment:指针参数为空
\retval ::HI_ERR_DMX_NOAVAILABLE_DATA  There is no valid data in the channel.CNcomment:通道中没有有效数据
\retval ::HI_ERR_DMX_TIMEOUT  The wait times out.CNcomment:等待超时
\retval ::HI_ERR_DMX_REACQUIRE_BUF  The data packet has been obtained.CNcomment:重复请求数据包
\see \n
 N/A.CNcomment:无
*/
...
/**
\brief Releases the buffers for storing data packets after data packets are processed.CNcomment:处理完数据包的数据后，释放数据包占用的缓冲空间
\attention \n
...
\param[in] hChannel        Channel handl.CNcomment:通道句柄。
\param[in] u32ReleaseNum   Number of released buffers.CNcomment:释放数据包个数
\param[in] pstBuf          Pointer to the number of released buffers. The number depends on the value of u32ReleaseNum.CNcomment:指针类型，指向释放缓冲区信息数组，长度由u32ReleaseNum决定
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HI_FAILURE  Calling this API fails.CNcomment:API系统调用失败
\retval ::HI_ERR_DMX_NOT_INIT  The DEMUX module is not initialized.CNcomment:模块没有初始化
\retval ::HI_ERR_DMX_INVALID_PARA  The input parameter is invalid. CNcomment:输入参数非法
