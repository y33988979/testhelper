...
/***************************** Macro Definition ******************************/
/** \addtogroup      H_3_1 */
/** @{ */  /** <!-- [Error code of the file DEMUX] CNcomment:【文件解析器错误码】 */
...
/** \addtogroup      H_2_1 */
/** @{ */  /** <!-- [Data structure of the file DEMUX] CNcomment:【文件解析器数据结构】 */
...
{
    /**
    \brief Check whether the entered protocol is supported. CNcomment:查找输入的协议是否支持
    \attention \n
    None
    \param[in] pszUrl file URL, such as http://. CNcomment:文件url，如http://

    \retval ::HI_SUCCESS The protocol is supported. CNcomment:支持该协议
    \retval ::HI_FAILURE The protocol is not supported. CNcomment:不支持该协议
    ...
    */
    ...
    /**
    \brief Open a file. CNcomment:打开文件
    \attention \n
    None
    \param [in]  pszUrl URL address. CNcomment:url地址
    \param [in]  eFlag open mode. CNcomment:打开方式
    \param [out] pUrlHandle protocol handle. CNcomment:协议句柄

    \retval ::HI_SUCCESS The file is successfully opened and the handle is valid. CNcomment:文件打开成功，句柄有效
    \retval ::HI_FAILURE The file fails to be opened. CNcomment:文件打开失败
    ...
    */
    ...
    /**
    \brief Read data of the specified size. CNcomment:读取指定大小数据
    \attention \n
    None
    \param [in] urlHandle file handle. CNcomment:文件句柄
    \param [in] s32Size size of the read data, in the unit of byte. CNcomment:读取的数据大小，字节为单位
    \param [out] pu8Buf data storage buffer. CNcomment:数据存储缓冲
    ...
    \retval ::Number greater than 0: The returned value indicates the number of read bytes, if less than s32Size, indicate the file ends.
                CNcomment:>0 返回的值是读取的字节数，如果实际读取的字节数小于s32Size，表明文件结束
    \retval ::HI_FAILURE The file ends. CNcomment:文件结束
    ...
    */
    ...
    /**
    \brief Seek to a specified location. HI_FAILURE is returned if the file does not support seek operations. CNcomment:seek到指定位置，对于不支持seek的文件，返回HI_FAILURE，
    \attention \n
    None
    \param [in] urlHandle file handle. CNcomment:文件句柄
    \param [in] s64Offsets number of sought bytes. CNcomment:seek字节数
    \param [in] s32Whence seeking flag. SEEK_CUR indicates seeking after s64Offsets is deviated from the current location, 
                ...
                HI_FORMAT_SEEK_FILESIZE返回文件大小。
    \retval ::Value other than HI_FAILURE: It indicates the number of start offset bytes in the file . CNcomment:非HI_FAILURE 文件起始偏移字节数
    \retval ::HI_FAILURE The seeking fails. CNcomment:seek失败
    ...
    */
    ...
    /**
    \brief Destroy the file handle. CNcomment:销毁文件句柄
    \attention \n
    None
    \param [in] urlHandle file handle. CNcomment:文件句柄

    \retval ::HI_SUCCESS The file handle is destroyed. CNcomment:文件句柄销毁成功
    \retval ::HI_FAILURE The parameter is invalid. CNcomment:参数非法
    ...
    */
    ...
    /**
    \brief Seek a specified stream.  CNcomment:seek指定的流
    \attention \n
    None
    \param [in] urlHandle file handle. CNcomment:文件句柄
    \param [in] s32StreamIndex stream index. CNcomment:流索引
    \param [in] s64Timestamp PTS, in the unit of ms. CNcomment:时间戳，单位ms
    \param [in] eFlags seeking flag. CNcomment:seek标记

    \retval ::HI_SUCCESS The seeking is successful. CNcomment:seek成功
    \retval ::HI_FAILURE The seeking fails. CNcomment:seek失败
    ...
    */
    ...
    /**
    \brief invoke operation. CNcomment:invoke操作
    \attention \n
    None
    \param [in] urlHandle file handle. CNcomment:文件句柄
    \param [in] u32InvokeId The value of the command is the type of HI_FORMAT_INVOKE_ID_E. The type of HI_FORMAT_INVOKE_ID_E only supports HI_FORMAT_INVOKE_PRE_CLOSE_FILE and HI_FORMAT_INVOKE_USER_OPE.
                CNcomment:值为::HI_FORMAT_INVOKE_ID_E类型, 仅支持HI_FORMAT_INVOKE_PRE_CLOSE_FILE/HI_FORMAT_INVOKE_USER_OPE
    \param [in] pArg the parameter corresponding to the invoke operation. CNcomment:invoke操作带的参数

    \retval ::HI_SUCCESS The operation is successful. CNcomment:成功
    \retval ::HI_FAILURE The operation fails. CNcomment:失败
...
typedef struct hiFORMAT_DEMUXER_FUN_S
{
    ...
    \brief Check whether the entered file format is supported. The value 0 indicates that the file format is supported and other values indicate that the file format is not supported.
           CNcomment:查找输入的文件格式是否支持，返回0支持，其他不支持
    \attention \n
    None
    \param [in] pszFileName file name, that is, path+file name. CNcomment:文件名称，路径+文件名
    \param [in] pstProtocol interface of the protocol. The URL can be open and the file data can be read by using this interface. CNcomment:协议操作接口，通过该接口可以打开url，读取文件数据

    \retval ::HI_SUCCESS The file format is supported. CNcomment:支持该文件格式
    \retval ::HI_FAILURE The file format is not supported. CNcomment:不支持该文件格式
    ...
    */
    ...
    /**
    \brief Open a specified file. CNcomment:打开指定文件
    \attention \n
    None
    \param [in] pszFileName file path. CNcomment:文件路径
    \param [in] pstProtocol protocol operation interface. You can specify the protocol operation interface by using this parameter if no protocol operation interface is configured for a registered dynamic link library (DLL).
                CNcomment:协议操作接口，注册的动态库如果没有协议操作接口可以通过该方式指定
    \param [out] pFmtHandle handle of the output file DEMUX. CNcomment:输出文件解析器句柄

    \retval ::HI_SUCCESS The file is opened successfully and the DEMUX handle is valid. CNcomment:文件打开成功，解析器句柄有效
    \retval ::HI_FAILURE The file fails to be opened. CNcomment:文件打开失败
    ...
    */
    ...
    /**
    \brief Look up the streams in the file. CNcomment:查找文件中的流信息
    \attention \n
    ...
    3. 只有fmt_open,fmt_find_stream函数都返回HI_SUCCESS，才认为文件打开成功

    \param [in] fmtHandle file handle returned by the functions. CNcomment:fmt_open函数返回的文件句柄
    \param [in] pArg expand parameter. CNcomment:扩展参数

    \retval ::HI_SUCCESS get the valid stream information successfully. CNcomment:文件中有有效的音视频或字幕流信息
    \retval ::HI_FAILURE no stream in the file. The file cannot be played. CNcomment:文件中没有任何流信息，该文件不可播放
    ...
    */
    ...
    \brief Obtain the file information. Set streamindex to HI_FORMAT_INVALID_STREAM_ID if a media file does not contain video streams.
           CNcomment:获取文件信息，如果媒体文件不带视频流，则需将视频streamindex设置为HI_FORMAT_INVALID_STREAM_ID
    \attention \n
    None
    \param [in] fmtHandle file handle. CNcomment:文件句柄
    \param [out] pstFmtInfo file information. CNcomment:文件信息

    \retval ::HI_SUCCESS The operation is successful. CNcomment:操作成功
    \retval ::HI_FAILURE The parameters are invalid. CNcomment:参数非法
    ...
    */
    ...
    /**
    \brief Read one frame data. The read data must be released by calling the fmt_free interface. CNcomment:读取一帧数据，读取数据必须调用fmt_free释放
    \attention \n
    If the read frame data has no PTS, set the PTS to HI_FORMAT_NO_PTS.  CNcomment:读取的帧数据如果没有时间戳，将时间戳值设置为HI_FORMAT_NO_PTS
    \param [in] fmtHandle file DEMUX handle. CNcomment:文件解析器句柄
    \param [in/out] pstFmtFrame media data. CNcomment:媒体数据信息

    \retval ::HI_SUCCESS The operation is successful. CNcomment:操作成功
    \retval ::HI_FAILURE The parameters are invalid. CNcomment:参数非法
    \retval ::HI_FORMAT_ERRO_ENDOFFILE The file ends. CNcomment:文件结束
    ...
    */
    ...
    /**
    \brief Release frame data. CNcomment:释放帧数据
    \attention \n
    None
    \param [in] fmtHandle file DEMUX handle. CNcomment:文件解析器句柄
    \param [in] pstFmtFrame media data read by the fmt_read interface. CNcomment:fmt_read接口读取的媒体数据信息
    ...
    */
    ...
    /**
    \brief Invoke the interface with the DEMUX and protocol. CNcomment:与解析器、协议交互操作接口
    \attention \n
    None
    \param [in] fmtHandle handle of the DEMUX. CNcomment:文件解析器句柄
    \param [in] u32InvokeId command of invoking. The value is HI_FORMAT_INVOKE_ID_E. CNcomment:交互命令，值为HI_FORMAT_INVOKE_ID_E
    \param [in/out] pArg the parameter corresponding to the command. CNcomment:命令对应的参数

    \retval ::HI_SUCCESS The invoke operation is successful. CNcomment:操作成功
    \retval ::HI_FAILURE The parameter is invalid. CNcomment:参数非法
    ...
    */
    ...
    \brief Seek frame data in a file based on the entered PTS. Seek frame data in a sequence from the current location to the file header if s64pts is set to 0.
           CNcomment:seek到指定pts，s64pts为0表示seek到文件头
    \attention \n
    None
    \param [in] fmtHandle file DEMUX handle. CNcomment:文件解析器句柄
    \param [in] s32StreamIndex stream index. CNcomment:流索引
    \param [in] s64Pts PTS, in the unit of ms. CNcomment:时间戳，单位ms
    \param [in] eFlag seeking flag. CNcomment:seek标记

    \retval ::HI_SUCCESS The operation is successful. CNcomment:操作成功
    \retval ::HI_FAILURE The seeking operation is not supported. CNcomment:不支持seek操作
    \retval ::HI_FORMAT_ERRO_ENDOFFILE end of file. CNcomment:文件结束
...
CNcomment:seek到指定位字节数的位置，对于裸码流文件，可以通过该接口实现跳跃
    \attention \n
    None
    \param [in] fmtHandle file DEMUX handle. CNcomment:文件解析器句柄
    \param [in] s64Offsets number of sought bytes. CNcomment:seek字节数
    \param [in] s32Whence seek flag, such as SEEK_CUR. CNcomment:seek标示，如SEEK_CUR

    \retval ::Value other than HI_FAILURE: It indicates the number of bytes deviated by the file pointer after seeking. CNcomment:非HI_FAILURE seek后，文件指针偏移字节数
    \retval ::HI_FAILURE The seeking operation is not supported. CNcomment:不支持seek操作
    \retval ::HI_FORMAT_ERRO_ENDOFFILE end of file. CNcomment:文件结束
    ...
    */
    ...
    /**
    \brief Disable the file DEMUX. CNcomment:关闭文件解析器
    \attention \n
    None
    \param [in] fmtHandle file DEMUX handle. CNcomment:文件解析器句柄
