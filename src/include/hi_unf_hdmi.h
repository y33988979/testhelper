...
{
    HI_U32                         u32RegistrationId;       /**CNcomment:< 注册ID号 */
...
/** \addtogroup      H_1_2_7 */
/** 
\brief the whole initialization of the hdmi. CNcomment:HDMI接口驱动软件初始化
\attention  this func should be called before vo_init and after disp_init. CNcomment:必须在DISP驱动Setup之后和VO驱动Setup之前打开
\param[in] pstHdmiPara  the initial params. CNcomment:初始化参数.
\the memeber: pfnHdmiEventCallback of HI_UNF_HDMI_INIT_PARA_S should be inited in one thread. and only be effectived in the first thread,other process can't get the callback event. CNcomment:子参数:pfnHdmiEventCallback 该hdmi回调事件在多线程中只有第一个调用进程有效，其他进程不能获得回调事件。
\retval HI_SUCCESS  success. CNcomment:成功
\retval please refer to the err code definitino of mpi.CNcomment:请参考MPI错误码
\see \n
CNcomment:无
*/
...
/** 
\brief deinit the hdmi. CNcomment:HDMI接口去初始化
\attention  this must be called after vo exited and before disp  exited . CNcomment:必须在VO驱动Exit之后和DISP驱动Exit之前打开
\param CNcomment:无
\retval HI_SUCCESS      success.CNcomment:成功
\retval please refer to the err code definitino of mpi.CNcomment:请参考MPI错误码
\see \n
...
/** 
\brief create hdmi device. CNcomment:创建HDMI设备
\attention \n
\param[in] enHdmi  hdmi device id.CNcomment:要打开的HDMI设备 
\retval HI_SUCCESS     success.  CNcomment:成功
\retval please refer to the err code definitino of mpi.CNcomment:请参考MPI错误码
\see \n
CNcomment:无
...
/** 
\brief close the handler created by  HI_UNF_HDMI_Open. CNcomment:销毁由HI_UNF_HDMI_Open创建的句柄
\attention \n
\param[in] enHdmi  hdmi device id. CNcomment:HDMI设备ID
\retval HI_SUCCESS     success.  CNcomment:成功
\retval please refer to the err code definitino of mpi.CNcomment:请参考MPI错误码
\see \n
CNcomment:无
...
/** 
\brief to get the capability of sink connect to stbbox. CNcomment:查询获取HDMI Sink设备的能力集
\attention \n
\param[in] enHdmi   hdmi device id.CNcomment:HDMI设备ID
\param[out] pCapability  the capability of the sink .CNcomment:SINK能力集
\retval HI_SUCCESS       success.  CNcomment:成功
\retval please refer to the err code definitino of mpi. CNcomment:请参考MPI错误码
...
/** 
\brief set the attr of given hdmi interface .CNcomment:设置HDMI接口属性。
\attention \n
\param[in] enHdmi      hdmi device id.CNcomment:HDMI设备ID
\param[in] pstAttr     the attr of given hdmi interface.CNcomment:HDMI接口属性
\retval HI_SUCCESS     success.  CNcomment:成功
\retval please refer to the err code definitino of mpi.CNcomment:请参考MPI错误码
...
/** 
\brief get the current attr of the give hdmi interface. CNcomment:查询HDMI接口当前属性
\attention \n
\param[in] enHdmi hdmi device id.CNcomment: HDMI设备ID
\param[out] pstAttr     the attr of given hdmi interface. CNcomment:HDMI接口属性
\retval HI_SUCCESS      success. CNcomment:成功
\retval please refer to the err code definitino of mpi.CNcomment:请参考MPI错误码
\see HI_UNF_HDMI_ATTR_S\n
CNcomment:无
...
/** 
\brief get the cec working status. CNcomment:获取CEC状态
\attention  user can get the cec working status through this func. CNcomment:客户可以通过该接口获取CEC工作状态\n
\param[in] enHdmi       hdmi device id.CNcomment:HDMI设备ID
\param[in] pStatus     get cec working status. CNcomment:CEC 状态
\retval HI_SUCCESS     success. CNcomment:成功
\retval please refer to the err code definitino of mpi.CNcomment:请参考MPI错误码
...
/** 
\brief send the cec data.CNcomment:发送CEC 命令
\attention \n
\param[in] enHdmi   hdmi device id. CNcomment:HDMI设备ID
\param[in] pCECCmd      the cec cmd data.CNcomment:Cec Command 内容
\retval HI_SUCCESS     success. CNcomment:成功
\retval please refer to the err code definitino of mpi.CNcomment:请参考MPI错误码
\see ::HI_UNF_HDMI_CEC_CMD_S\n
CNcomment:无
...
/** 
\brief get the cec data received. CNcomment:接收的CEC 命令
\attention \n
\param[in] enHdmi   hdmi device id. CNcomment:HDMI设备ID
\param[in] pCECCmd    the cec cmd data. CNcomment:Cec Command 内容
...
/** 
\brief  set and send infoframe. CNcomment:设置并发送InfoFrame
\attention \n
\param[in] enHdmi       hdmi device id.CNcomment:HDMI设备ID
\param[in] pstInfoFrame the inforframe content.CNcomment:InfoFrame内容
\retval HI_SUCCESS      success. CNcomment:成功
\retval please refer to the err code definitino of mpi.CNcomment:请参考MPI错误码
\see \n
CNcomment:无
...
/** 
\brief get the infoframe infor. CNcomment:获取正在发送的InfoFrame的信息
\attention \n
\param[in] enHdmi       hdmi device id. CNcomment:HDMI设备ID
\param[in] enInfoFrameType the info frame type such as avi or audio or gcp etc. CNcomment:InfoFrame类型
\param[in] pstInfoFrame   the inforframe content.CNcomment:InfoFrame内容
\retval HI_SUCCESS      success.CNcomment:成功
\retval please refer to the err code definitino of mpi.CNcomment:请参考MPI错误码
\see \n
...
CNcomment:启动HDMI接口。一般在HI_UNF_HDMI_SetAttr之后调用
\param[in] enHdmi   hdmi device id.CNcomment:HDMI设备ID
\retval HI_SUCCESS     success. CNcomment:成功
\retval please refer to the err code definitino of mpi.CNcomment:请参考MPI错误码
\see \n
...
CNcomment:当HDMI线被拔除后，HDMI回调函数应该调用该函数。
\param[in] enHdmi  hdmi device id. CNcomment:HDMI设备ID
\retval HI_SUCCESS     success. CNcomment:成功
\retval please refer to the err code definitino of mpi.CNcomment:请参考MPI错误码
\see \n
...
/** 
\brief set the deep color mode .CNcomment:设置HDMI DeepColor模式
\attention \n
\param[in] enHdmi   hdmi device id. CNcomment:HDMI设备ID
\param[in] enDeepColor deep color mode,please refer to the HI_UNF_HDMI_DEEP_COLOR_E definiton.CNcomment:DeepColor模式，请参考::HI_UNF_HDMI_DEEP_COLOR_E  
\retval HI_SUCCESS     success. CNcomment:成功
\retval please refer to the err code definitino of mpi.CNcomment:请参考MPI错误码
\see \n
CNcomment:无
...
/** 
\brief switch the xvycc on or off. CNcomment:设置HDMI xvYCC 模式
\attention \n
\param[in] enHdmi   hdmi device id.CNcomment:HDMI设备ID
\param[in] bEnalbe   whether to enable xvycc mode or not .CNcomment:是否使能xvYCC模式
\retval HI_SUCCESS    success.  CNcomment:成功
\retval please refer to the err code definitino of mpi.CNcomment:请参考MPI错误码
\see \n
CNcomment:无
...
/** 
\brief switch the mute of av  on or off. CNcomment:设置HDMI AV mute 模式
\attention \n
\param[in] enHdmi   hdmi device id.CNcomment:HDMI设备ID
\param[in] bAvMute   whether to mute the av.CNcomment:是否mute AV
\retval HI_SUCCESS     success. CNcomment:成功
\retval please refer to the err code definitino of mpi.CNcomment:请参考MPI错误码
\see \n
无
...
/** 
\brief get the edid information forcelly.CNcomment:强制获取EDID原始数据,该接口为特定平台需要获取原始EDID的接口
\attention \n
\param[in] enHdmi   hdmi device id.CNcomment:HDMI设备ID
\param[in] *u8Edid  the buffer allocated externally, buffer size must be 512. CNcomment:获取原始EDID数据buffer。EDID版本不同，读出来的数据不同，取上限buffer 大小为512,用户需自己分配好;
\param[in] *u32EdidLength  the data lenth of  original edid. CNcomment:获取原始EDID数据长度
\retval HI_SUCCESS   success.   CNcomment:成功
\retval please refer to the err code definitino of mpi.CNcomment:请参考MPI错误码
\see \n
CNcomment:无
