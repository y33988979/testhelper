...
/**
\file
\brief Describes the header file of the text module.CNcomment:text模块头文件
...
/**Font attributes*//**CNcomment: 字体属性 */
typedef struct
{
...
/** @{ */  /** <!—[text module]*//**CNcomment: <!-- 【text模块】 */

/**
\brief Creates a text output object.CNcomment: 创建文本输出对象
\attention \n
...
-byte character set library is not used.CNcomment: 单字节字符集字体文件，为空表示不使用单字节字符集字库
...
cannot be empty concurrently.CNcomment: 多字节字符集字体文件，为空表示不使用多字节字符集字库，但是不能与单字节字符文件同时为空。
\param[out]  pTextOut Handle of a text output object.CNcomment: 文本输出对象句柄
...
/** 
\brief Destroys a text output object.CNcomment:销毁文本输出对象
\attention \n
N/A.CNcomment:无
\param[in] TextOut Handle of a text output object.CNcomment:文本输出对象句柄
...
/** 
\brief set the input charset:设置输入字符集
\attention \n
N/A.CNcomment:无
\param[in] TextOut Handle of a text output object.CNcomment:文本输出对象句柄
\param[out] CharSet  charset of input text.CNcomment:输入文本字符集合
...
/** 
\brief Obtains the attributes of a text output object.CNcomment:获取文本输出对象属性
\attention \n
N/A.CNcomment:无
\param[in] TextOut Handle of a text output object.CNcomment:文本输出对象句柄
\param[out] pTextOutAttr  Attributes of a text output object.CNcomment:文本输出对象属性
...
/** 
\brief Obtains the width and height of text contents.CNcomment:获取文本内容的宽高
\attention \n
N/A.CNcomment:无
\param[in] TextOut Handle of a text output object.CNcomment:文本输出对象句柄
\param[in] pText Text contents.CNcomment:文本内容
\param[out] pWidth Width of text contents. It cannot be empty.CNcomment:文本内容宽度，不可为空
\param[out] pHeight Height of text contents. It cannot be empty.CNcomment:文本内容高度，不可为空
...
/** 
\brief Sets to display the background color of a text output object.CNcomment:设置文本输出对象的背景色是否显示
\attention \n
N/A.CNcomment:无
\param[in] TextOut Handle of a text output object.CNcomment:文本输出对象句柄
\param[in] bTransparent  Whether to display the background color of a text output object. If the value is set to 
HI_TRUE, the background color is not displayed. The default value is HI_TRUE.CNcomment:文本输出对象背景色是否显示，为HI_TRUE, 则不显示，默认为HI_TRUE
...
/** 
\brief Sets the background color of a text output object.CNcomment:设置文本输出对象的背景色
\attention \n
If the target surface is a palette, the color is the palette index.CNcomment:如果目标surface是调色板，则Color是调色板索引
\param[in] TextOut Handle of a text output object.CNcomment:文本输出对象句柄
\param[in] Color  Background color of a text output object.CNcomment:文本输出对象背景色，
...
/**
\brief Sets the font color of a text output object.CNcomment: 设置文本输出对象的字体颜色
\attention \n
If the target surface is a palette, the color is the palette index.CNcomment: 如果目标surface是调色板，则Color是调色板索引
\param[in] TextOut Handle of a text output object.CNcomment: 文本输出对象句柄
\param[in] Color  Font color of a text output object.CNcomment: 文本输出对象字体颜色
...
/** 
\brief Outputs text contents to a specified surface.CNcomment:输出文本内容到指定surface
\attention
N/A.CNcomment:无
\param[in] TextOut Handle of a text output object.CNcomment:文本输出对象句柄
...
surface origin.CNcomment:文本输出区域，为空表示从surface原点输出
...
styles and formatting.CNcomment:文本输出扩展接口,输出内容可以按照用户指定的排版进行输出。
\param[in] hTextOut Text output handle.CNcomment:文本输出句柄
\param[in] hSurface Surface handle.CNcomment:Surface句柄
\param[in] pText    Text contents ending with /0.CNcomment:以/0结尾的文本内容
\param[in] pRect    Text output region.CNcomment:文本输出区域
\param[in] Style    Styles and formatting including LAYOUT_LEFT, LAYOUT_RIGHT, and LAYOUT_HCENTER.CNcomment:排版格式，如LAYOUT_LEFT、LAYOUT_RIGHT、LAYOUT_HCENTER等
...
/**
\brief Calculates the number of characters in a specified text output region.CNcomment: 计算指定文本输出区域字符个数
\attention \n
N/A.CNcomment:无
\param[in] hTextOut Text output handle.CNcomment: 文本输出句柄
\param[in] pText    Text contents ending with /0.CNcomment: 以/0结尾的文本内容
\param[in] pRect    Text output region. The value cannot be empty.CNcomment: 文本输出区域,不可为空
\param[in] pLen      Pointer to the string length.CNcomment: 字符串长度指针
...
/** 
\brief Creates a font.CNcomment:创建字体
\attention \n
N/A.CNcomment:无
\param[in] phText Text output handle.CNcomment:文本输出句柄
\param[in] pInfo  Text information.CNcomment:文件信息

\retval ::HI_SUCCESS A font is created successfully.CNcomment:字体创建成功
\retval ::HI_FAILURE A font fails to be created.CNcomment:字体创建失败
\retval ::HIGO_ERR_INVPARAM
\retval ::HIGO_ERR_NOMEM
\retval ::HIGO_ERR_INVHANDLE
...
/** 
\brief Sets the style of a font, such as bold, italic, or normal.CNcomment:设置字体的风格，比如:粗体，斜体，正常体
\attention \n
N/A.CNcomment:无
\param[in] hTextOut Text output handle.CNcomment:文本输出句柄
\param[in] eStyle   Font style.CNcomment:字体风格
...
lines.CNcomment:设置字符显示水平间距，两个字符行水平间距
\attention \n
N/A.CNcomment:无

\param[in] hTextOut Text output handle.CNcomment:文本输出句柄
\param[in] u32Distance Horizontal spacing.CNcomment:水平间距

\retval ::HI_SUCCESS Success.CNcomment: 成功
\retval ::HIGO_ERR_INVHANDLE Failure.CNcomment:失败
...
/** 
\brief Sets the vertical and horizontal spacing.CNcomment:设置垂直水平间距
\attention \n
This API is available only when the font style is changed.CNcomment:只有换行风格才有效

\param[in] hTextOut Text output handle.CNcomment:文本输出句柄
\param[in] u32Distance Vertical spacing.CNcomment:垂直间距
...
/**
\brief Initializes the character module.CNcomment: 初始化字符模块
\attention \n
N/A.CNcomment: 无
...
/** 
\brief Deinitializes the character module.CNcomment:去初始化字符模块
\attention \n
N/A.CNcomment:无
\param N/A.CNcomment:无
\retval ::HI_SUCCESS Success.CNcomment:成功
\retval ::HIGO_ERR_NOTINIT Failure.CNcomment:失败
