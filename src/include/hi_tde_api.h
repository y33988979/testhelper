...
/** 
\brief Cancels an operation added in a TDE task. CNcomment:取消已经加入到TDE任务中的操作
\attention \n
Before calling this API, you must call HI_TDE2_Open to start a TDE device and call HI_TDE2_BeginJob to obtain a valid task handle.\n
...
\param[in] s32Handle Handle of a TDE task. CNcomment:任务句柄
\retval 0 success. CNcomment:成功
\retval ::HI_ERR_TDE_DEV_NOT_OPEN  Calling this API fails because no TDE device is started. CNcomment:TDE设备未打开，API调用失败
\retval ::HI_FAILURE  The specified task cannot be cancelled because it has been submitted. CNcomment:指定的任务已经提交无法取消
...
/** 
\brief Waits for the completion of a specified TDE task. CNcomment:等待指定的任务完成
\attention \n
 As a block interface, this API is blocked until the specified task is complete.\n
 CNcomment:此接口为阻塞接口，会阻塞等待指定的任务完成\n
 
\param[in] s32Handle Handle of a TDE task. CNcomment:任务句柄
\retval 0  The specified TDE task is not completed. CNcomment:指定的TDE任务未完成
\retval ::HI_ERR_TDE_DEV_NOT_OPEN  Calling this API fails because no TDE device is started. CNcomment:TDE设备未打开，API调用失败
\retval ::HI_ERR_TDE_INVALID_HANDLE  The handle of a TDE task is invalid. CNcomment:非法的任务句柄
\retval ::HI_ERR_TDE_QUERY_TIMEOUT  The specified task is not completed due to timeout. CNcomment:指定的任务超时未完成
\retval ::HI_ERR_TDE_UNSUPPORTED_OPERATION  The operation is not supported. CNcomment:不支持的操作
...
/** 
\brief Waits for the completion of all TDE tasks. CNcomment:等待TDE的所有任务完成
\attention \n
...
/** 
\brief Resets all statuses of the TDE. CNcomment:复位TDE所有状态
\attention \n
...
/** 
\brief Adds the fast copy operation to a TDE task. CNcomment:向任务中添加快速拷贝操作
\attention \n
...
\param[in] s32Handle  Handle of a TDE task. CNcomment:TDE任务句柄
\param[in] pstSrc  Source bitmap. CNcomment:源位图
\param[in] pstSrcRect  Operating area of the source bitmap. CNcomment:源位图操作区域
...
CNcomment:将源位图带缩放为目标位图指定大小，源和目标可为同一位图
\attention \n
...
\retval ::HI_ERR_TDE_INVALID_PARA  The parameter is invalid. CNcomment:无效的参数设置
\retval ::HI_ERR_TDE_NO_MEM  Adding an operation fails due to insufficient memory. CNcomment:内存不足，无法添加操作
\retval ::HI_ERR_TDE_NOT_ALIGNED  The start address or stride of the bitmap is not aligned based on pixel. CNcomment:位图起始地址或Stride没有按照像素对齐
\retval ::HI_ERR_TDE_MINIFICATION  The start address or stride of the bitmap is not aligned based on pixel. CNcomment:缩小倍数过大
\retval ::HI_ERR_TDE_NOT_ALIGNED  The start address of the color look-up table (CLUT) is not 4-byte aligned. CNcomment:Clut表的起始地址没有按照4byte对齐
\retval ::HI_ERR_TDE_UNSUPPORTED_OPERATION  The operation is not supported. CNcomment:不支持的操作配置
\retval ::HI_FAILURE  A system error or an unknown error occurs. CNcomment:系统错误或未知错误
\see \n
N/A
*/
...
/** 
\brief Adds the anti-flicker operation to a TDE task. CNcomment:向任务中添加抗闪烁操作 
\attention \n
...
\retval ::HI_FAILURE  A system error or an unknown error occurs. 系统错误或未知错误
\see \n
N/A
*/
...
CNcomment:向任务中添加对光栅位图进行有附加操作的填充搬移操作。实现在surface上画点、画线、色块填充或内存填充等功能
\attention \n
Before calling this API, you must call HI_TDE2_Open to start a TDE device and call HI_TDE2_BeginJob to obtain a valid task handle.\n
...
\param[in] s32Handle  Handle of a TDE task. CNcomment:TDE任务句柄
\param[in] pstForeGround  Foreground bitmap. CNcomment:前景位图
\param[in] pstForeGroundRect  Operating area of the foreground bitmap. CNcomment:前景位图的操作区域
\param[in] pstDst  Target bitmap. CNcomment:目标位图
\param[in] pstDstRect  Operating area of the target bitmap. CNcomment:目标位图的操作区域
\param[in] pstFillColor  Structure of fill colors. CNcomment:填充色结构体
\param[in] pstOpt  Structure of operation attributes. CNcomment:操作属性结构体
\retval 0 success. CNcomment:成功
\retval ::HI_ERR_TDE_DEV_NOT_OPEN  Calling this API fails because no TDE device is started. CNcomment:TDE设备未打开，API调用失败
\retval ::HI_ERR_TDE_NULL_PTR  The parameter contains a null pointer. CNcomment:参数中有空指针错误
\retval ::HI_ERR_TDE_INVALID_HANDLE  The handle of a TDE task is invalid. CNcomment:非法的任务句柄
...
CNcomment:向任务中添加对光栅位图进行Mask Rop搬移操作。根据Mask位图实现前景位图和背景位图ROP的效果
\attention \n
Before calling this API, you must call HI_TDE2_Open to start a TDE device and call HI_TDE2_BeginJob to obtain a valid task handle.\n
...
\param[in] pstForeGround  Foreground bitmap. CNcomment:前景位图
\param[in] pstForeGroundRect  Operating area of the foreground bitmap. CNcomment:前景位图操作区域
\param[in] pstMask  Mask bitmap. CNcomment:Mask位图
\param[in] pstMaskRect  Operating area of the mask bitmap. CNcomment:Mask位图操作区域
\param[in] pstDst  Target bitmap. CNcomment:目标位图
\param[in] pstDstRect  Operating area of the target bitmap. CNcomment:目标位图操作区域
\param[in] enRopCode_Color  ROP code of a color component. CNcomment:颜色分量的ROP运算码
\param[in] enRopCode_Alpha  ROP code of the alpha component. CNcomment:Alpha分量的ROP运算码
\retval 0 success. CNcomment:成功
\retval ::HI_ERR_TDE_DEV_NOT_OPEN  Calling this API fails because no TDE device is started. CNcomment:TDE设备未打开，API调用失败
\retval ::HI_ERR_TDE_NULL_PTR  The parameter contains a null pointer. CNcomment:参数中有空指针错误
\retval ::HI_ERR_TDE_INVALID_HANDLE  The handle of a TDE task is invalid. CNcomment:非法的任务句柄
...
CNcomment:向任务中添加对光栅位图进行Mask Blend搬移操作。根据Mask位图实现前景位图和背景位图带Mask位图的叠加效果
\attention \n
Before calling this API, you must call HI_TDE2_Open to start a TDE device and call HI_TDE2_BeginJob to obtain a valid task handle.\n
...
\param[in] pstDst  Target bitmap. CNcomment:目标位图
\param[in] pstDstRect  Operating area of the target bitmap. CNcomment:目标位图操作区域
\param[in] u8Alpha  Global alpha value during alpha blending. CNcomment:Alpha叠加时的全局Alpha值
\param[in] enBlendMode  Alpha blending mode. CNcomment:Alpha叠加模式选择
\retval 0 success. CNcomment:成功
\retval ::HI_ERR_TDE_DEV_NOT_OPEN  Calling this API fails because no TDE device is started. CNcomment:TDE设备未打开，API调用失败
\retval ::HI_ERR_TDE_NULL_PTR  The parameter contains a null pointer. CNcomment:参数中有空指针错误
\retval ::HI_ERR_TDE_INVALID_HANDLE  The handle of a TDE task is invalid. CNcomment:非法的任务句柄
...
/** 
\brief  Adds the rotation operation to a TDE task. CNcomment:向任务中添加旋转操作
\attention \n
...
\param[in] pstSrcRect  Operating area of the source bitmap. CNcomment:源位图操作区域
\param[in] pstDst  Target bitmap. CNcomment:目标位图
\param[in] s32DstPointX  Horizontal coordinate of the start point of the operating area in the target bitmap. CNcomment:目标位图操作区域的起始点横坐标
\param[in] s32DstPointY  Vertical coordinate of the start point of the operating area in the target bitmap. CNcomment:目标位图操作区域的起始点纵坐标
\param[in] enRotateAngle  Rotate angle. CNcomment:旋转的角度
\retval 0 success. CNcomment:成功
\retval ::HI_ERR_TDE_DEV_NOT_OPEN  Calling this API fails because no TDE device is started. CNcomment:TDE设备未打开，API调用失败
\retval ::HI_ERR_TDE_INVALID_HANDLE  The handle of a TDE task is invalid. CNcomment:非法的任务句柄
\retval ::HI_ERR_TDE_NULL_PTR  The parameter contains a null pointer. CNcomment:参数中有空指针错误
...
/** 
\brief Sets the anti-flicker level. CNcomment:设置抗闪烁级别
\attention \n
N/A
\param[in] enDeflickerLevel  Anti-flicker level. CNcomment:抗闪烁系数级别
\retval 0 success. CNcomment:成功
\retval ::
...
/** 
\brief Obtains the anti-flicker level.CNcomment: 获取抗闪烁级别
\attention \n
N/A
\param[in] pDeflickerLevel  Anti-flicker level. CNcomment:抗闪烁级别
\retval 0 success. CNcomment:成功
\retval ::
...
/** 
\brief Obtains the alpha judgment threshold. CNcomment:获取alpha判决阈值
\attention \n
N/A
\param[in] pu8ThresholdValue  Judgment threshold. CNcomment:判决阈值
\retval 0 success. CNcomment:成功
\retval ::
...
/** 
\brief Obtains the enable status of the alpha judgment. CNcomment:获取alpha判决开关
\attention N/A
\param[in]  p_bEnAlphaThreshold  Whether to enable the alpha judgment. CNcomment:alpha判决开关
\retval 0 success. CNcomment:成功
\retval ::
...
/** 
\brief Fills in the pattern. CNcomment:模式填充
\attention N/A
\param[in] s32Handle  Handle of a TDE task. CNcomment:任务句柄
\param[in] pstBackGround  Background bitmap. CNcomment:背景位图信息
\param[in] pstBackGroundRect  Operating rectangle of the background bitmap. CNcomment:背景操作矩形
\param[in] pstForeGround  Foreground bitmap. CNcomment:前景位图信息
\param[in] pstForeGroundRect  Operating rectangle of the foreground bitmap. CNcomment:前景位操作矩形
\param[in] pstDst  Target bitmap. CNcomment:目标位图信息
\param[in] pstDstRect  Operating rectangle of the target bitmap. CNcomment:目标操作矩形
\param[in] pstOpt  Options for pattern filling. CNcomment:模式填充操作选项
\retval 0 success. CNcomment:成功
\retval ::
...
/** 
\brief Enables or disables the regional anti-flicker function. CNcomment:使能/去使能局部抗闪烁
\attention N/A
\param[in] bRegionDeflicker  Enable/disable flag. CNcomment:使能/去使能标志
\retval 0 success. CNcomment:成功
\retval ::
