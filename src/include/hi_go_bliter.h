...
/**  
\brief Draws a line segment. Cropping is not supported.CNcomment:绘制线段，不支持裁减
\attention \n
Cropping is not supported. Users must keep the entire line within the surface region.
CNcomment:不支持裁减，使用者必须保证整条直线在surface区域内

\param[in] Surface Destination surface handle.CNcomment:目的surface句柄
\param[in] x0 Horizontal coordinate of the start point.CNcomment:起点x坐标
\param[in] y0 Vertical coordinate of the start point.CNcomment:起点y坐标
\param[in] x1 Horizontal coordinate of the end point.CNcomment:终点x坐标
\param[in] y1 Vertical coordinate of the end point .CNcomment:终点y坐标 
\param[in] color Line segment color.CNcomment:线段颜色
...
/**  
\brief Draws an ellipse.CNcomment:绘制椭圆
\attention \n
Cropping is not supported. Users must keep the entire ellipse within the surface region.
CNcomment:不支持裁减，使用者必须保证整个椭圆在surface区域内

\param[in] Surface Destination surface handle.CNcomment:目的surface句柄
\param[in] sx Horizontal coordinate of the ellipse center.CNcomment:圆心x坐标
\param[in] sy Vertical coordinate of the ellipse center.CNcomment:圆心y坐标
\param[in] rx X axis radius .CNcomment:x轴半径 
\param[in] ry Y axis radius.CNcomment:y轴半径
\param[in] color Ellipse color.CNcomment:椭圆颜色
...
/**  
\brief Draws a circle. Cropping is not supported.CNcomment:绘制圆，不支持裁减
\attention \n
Cropping is not supported. Users must keep the entire circle within the surface region.
CNcomment:不支持裁减，使用者必须保证整个圆在surface区域内

\param[in] Surface Destination surface handle.CNcomment:目的surface句柄
\param[in] x Horizontal coordinate of the circle center.CNcomment:圆心x坐标
\param[in] y Vertical coordinate of the circle center.CNcomment:圆心y坐标
\param[in] r Radius.CNcomment:半径
\param[in] color Circle color. The circle is filled in with 32-bit colors.CNcomment:圆颜色, 颜色按照32位色进行填充
...
/**  
\brief Draws a rectangle.CNcomment:绘制矩形
\attention \n
...
\param[in] Surface Destination surface handle.CNcomment:目的surface句柄
\param[in] pRect Rectangle region.CNcomment:矩形区域
\param[in] color Rectangle color.CNcomment:矩形颜色
...
/**  
\brief Performs the mask ROP or mask blend transfer on the raster bitmap.CNcomment:对光栅位图进行Mask Rop或者Mask Blend搬移操作
\attention \n
...
\param[in] SrcSurface Source surface handle.CNcomment:源surface句柄
\param[in] pSrcRect Source rectangle.CNcomment:源矩形
\param[in] DstSurface Destination surface handle.CNcomment:目标surface句柄
\param[in] pDstRect Destination rectangle.CNcomment:目标矩形
\param[in] MaskSurface Mask surface handle.CNcomment:MASK surface句柄
\param[in] pMaskRect Mask rectangle.CNcomment:MASK矩形
\param[in] pOpt Operation option.CNcomment:操作选项
...
/**  
\brief Transfers bit blocks. During the transfer, CSC is supported.CNcomment:位块搬移，在搬移过程中，可以实现色彩空间转换
\attention \n
Only the YUV-to-RGB CSC is supported. \n
The operations of colorkey, alpha, ROP, and colorkey+ROP are supported.\
CNcomment: 色彩空间转换仅支持YUV到RGB转换 \n
...
\param[in] pDstRect Destination region for transfer. If the parameter is not set, it indicates the entire destination 
surface.CNcomment:搬移的目的区域，为空表示整个目标surface区域
\param[in] pBlitOpt Blending mode. If the parameter is not set, default settings are used. CNcomment:混合方式，参数为空使用默认参数操作
...
/**  
\brief Transfers bit blocks. During the transfer, CSC and scaling are supported. CNcomment:位块搬移，在搬移过程中，可以实现色彩空间转换, 缩放
\attention \n
Only the YUV-to-RGB CSC is supported. \n
The operations of colorkey, alpha, ROP, and colorkey+ROP are supported.\
CNcomment:色彩空间转换仅支持YUV到RGB转换 \n
...
/** 
\brief  Fills in the pattern.CNcomment:进行模式填充
\attention \n
\param[in] SrcSurface  Source surface handle. CNcomment:源surface句柄
\param[in] pSrcRect  Source region for transfer. If the parameter is not set, it indicates the entire source surface.CNcomment:搬移的源区域，为空表示整个源surface区域
\param[in] DstSurface  Destination surface handle. CNcomment:目的surface句柄
\param[in] pDstRect  Destination region for transfer. If the parameter is not set, it indicates the entire destination 
surface.CNcomment:搬移的目的区域，为空表示整个目标surface区域
\param[in] pParOpt  Blending mode. If the parameter is not set, default settings are used. CNcomment:混合方式,参数为空使用默认参数操作
...
/** 
\brief Performs anti-flicker and transfer. This API is used for the second anti-flicker.CNcomment:抗闪烁搬移，这个接口用于第二次抗闪烁，
\attention \n
...
\param[in] SrcSurface  SrcSurface Source surface handle.CNcomment: 源surface句柄
\param[in] pSrcRect  pSrcRect Source region for transfer. If the parameter is not set, it indicates the entire source surface.CNcomment:搬移的源区域，为空表示整个源surface区域
\param[in] DstSurface  DstSurface Destination surface handle. CNcomment:目的surface句柄
\param[in] pDstRect  pDstRect Destination region for transfer. If the parameter is not set, it indicates the entire destination 
surface. CNcomment:搬移的目的区域，为空表示整个目标surface区域
\param[in] pDefOpt  pDefOpt Anti-flicker level. If this parameter is not set, it indicates the automatic level. CNcomment:抗闪烁级别选项，为空表示AUTO 级别
...
You can performs color space coversion, scale, or mirror, colorkey with alpha blending.CNcomment:该函数实现前景和背景叠加输出到目标功能，背景和目标surface必须位于RGB空间，在叠加过程中可以附加颜色空间转换、缩放、镜像等操作，不支持旋转。
\attention \n
Only the YUV-to-RGB CSC is supported. \n
...
\param[in] BckSurface background surface handle.CNcomment:背景surface句柄
\param[in] pBckRect backgound region for transfer. If the parameter is not set, it indicates the entire background surface.CNcomment:背景surface操作区域，为空表示整个背景surface区域
\param[in] ForSurface forground surface handle.CNcomment:前景surface句柄
\param[in] pForRect forground region for transfer. If the parameter is not set, it indicates the entire forground surface. 
surface.CNcomment:前景surface操作区域，为空表示整个前景surface区域
\param[in] DstSurface destination surface handle.CNcomment:目标surface句柄
