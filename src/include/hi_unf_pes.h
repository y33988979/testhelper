...
/** 
 * \file
 * \brief  Describes the information about the Packetized elementary stream parse information. CNcomment: 提供PES stream 解码的相关信息
 */
...
/**
\brief Open  PES module  CNcomment:打开PES模块
\attention \n
N/A
\param[in] stOpenParam Open parameter.CNcomment:PES  模块打开输入参数
\param[out] handle    Pointer to the handle of a pes instance .CNcomment:指针类型，打开PES模块所生成的PES实例句柄
...
/**
\brief  Close  PES module  CNcomment:关闭PES模块
\attention \n
N/A
\param[in] handle  the handle of a pes instance CNcomment:PES实例句柄
\retval ::HI_SUCCESS Success  CNcomment:成功
\retval ::HI_FAILURE  This API fails to be called due to system errors. CNcomment:出现系统错误，API调用失败
\see \n
N/A
...
/**
\brief PES  Obtain PES buffer.CNcomment:获取PES buffer
\attention \n
N/A
\param[in] handle  the handle of a pes instance CNcomment:PES实例句柄
\param[in] u32ReqSize: Required buffer size,it must smaller than whole PS Buffer.Recommend it's a multiple of  whole PS Buffer's 1/N CNcomment:需要的缓存大小，必须小于整个PS Buffer的大小。建议是整个大小的1/N
...
/**
\brief  After filling data,send it to PES process module.CNcomment: PES数据填完后，推送数据给PES模块处理
\attention \n
It must be called after HI_UNF_PES_GetBuffer  CNcomment:  必须与HI_UNF_PES_GetPSBuffer配对使用。
\param[in] handle  the handle of a pes instance CNcomment:PES实例句柄
\param[in] u32ValidDatalen : Valid data length  CNcomment:送入的数据长度
\retval ::HI_SUCCESS Success  CNcomment:成功
\retval ::HI_FAILURE  This API fails to be called due to system errors. CNcomment:出现系统错误，API调用失败
\see \n
N/A
...
/**
\brief Obtain PES stream information CNcomment: 获取PES流信息.
\attention \n
N/A
\param[in] handle  the handle of a pes instance CNcomment:PES实例句柄
\param[out] pstruPSInfo  Pointer to the returned PES stream information CNcomment: 返回PES码流信息指针
\retval ::HI_SUCCESS Success  CNcomment:成功
\retval ::HI_FAILURE  This API fails to be called due to system errors. CNcomment:出现系统错误，API调用失败
\see \n
N/A
...
/**
\brief Set Video PES stream id CNcomment: 设置视频PES流ID.
\attention \n
N/A
\param[in] handle  the handle of a pes instance CNcomment:PES实例句柄
\param[in] videostreamid  Video PES stream id CNcomment: 视频PES流ID
\retval ::HI_SUCCESS Success  CNcomment:成功
\see \n
N/A
*/
...
/**
\brief Set Audio PES stream id CNcomment: 设置音频PES流ID.
\attention \n
N/A
\param[in] handle  the handle of a pes instance CNcomment:PES实例句柄
\param[in] audiostreamid  Audio PES stream id CNcomment: 音频PES流ID
\retval ::HI_SUCCESS Success  CNcomment:成功
\see \n
N/A
*/
