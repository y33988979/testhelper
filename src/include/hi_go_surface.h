/**
 \file
 \brief Describes the header file of the surface module. CNcomment:Surface模块头文件
...
/***************************** Macro Definition ******************************/
/** \addtogroup      H_3_3 */
/** @{ */  /** <!-- [Error codes of the surface module] */
...
/** 
\brief Initializes the surface module. CNcomment:初始化Surface
\attention \n
When ::HI_GO_Init is called, this API is also called.
CNcomment: ::HI_GO_Init已经包含对该接口的调用
\param 无
...
/** 
\brief Deinitializes the surface module. CNcomment:去初始化Surface
\attention \n
When ::HI_GO_Deinit is called, this API is also called.
CNcomment: ::HI_GO_Deinit已经包含对该接口的调用
\param N/A
...
/** 
\brief Sets the alpha value of a surface. CNcomment:设置surface的alpha值
\attention \n
N/A
\param[in]  Surface Surface handle. CNcomment:Surface句柄
\param[in]  Alpha Alpha value, ranging from 0 to 255. The value 0 indicates transparent, and the value 255 indicates opaque. CNcomment:Alpha值，范围是0-255。0表示透明，255表示不透明
...
/** 
\brief Obtains the alpha value of a surface. CNcomment:获取surface的alpha值
\attention \n
N/A
\param[in]  Surface Surface handle. CNcomment:Surface句柄
\param[out] pAlpha Pointer to the received alpha value. The pointer cannot be null. CNcomment:接收alhpa值的空间指针，不能为空
...
/** 
\brief Enables or disables the colorkey of a surface. CNcomment:设置是否使能surface的colorkey
\attention \n
N/A
\param[in] Surface Surface handle. CNcomment:Surface句柄
\param[in] Enable Colorkey enable. HI_TRUE: enabled; HI_FALSE: disabled. CNcomment:是否使能colorKey。HI_TRUE：使能；HI_FALSE：不使能
...
/** 
\brief Sets the colorkey value of a surface. CNcomment:设置surface的colorKey值
\attention \n
N/A
\param[in] Surface Surface handle. CNcomment:Surface句柄
\param[in] ColorKey Colorkey value. For the RGB format, the colorkey is padded with 32-bit colors. For the CLUT format, the colorkey is padded with color index. CNcomment:Colorkey值, 如果是RGB格式就使用全部按照32bit来进行填充，如果是CLUT格式就使用颜色索引来填充。
...
/** 
\brief Obtains the colorkey value of a surface. CNcomment:获取surface的colorkey值
\attention \n
N/A
\param[in]  Surface Surface handle. CNcomment:Surface句柄
\param[out] pColorKey Pointer to the received colorkey value. The value cannot be empty. CNcomment:接收colorkey值的空间指针，不可为空
...
/** 
\brief Sets the palette of a surface. CNcomment:设置Surface的调色板
\attention \n
N/A
\param[in] Surface Surface handle. CNcomment:Surface句柄
\param[in] Palette Palette. CNcomment:调色板
...
/** 
\brief Obtains the palette of a surface. CNcomment:获取surface的调色板
\attention \n
N/A
\param[in]  Surface Surface handle. CNcomment:Surface句柄
\param[out] Palette Pointer to the received palette. CNcomment:接收调色板的空间指针 
...
/** 
\brief Locks a surface and obtains its memory pointer. CNcomment:锁定surface，获取其内存指针
\attention \n
...
不能对同一surface重复锁定
\param[in] Surface Surface handle. CNcomment:Surface句柄
\param[out] pData Pixel format information related to memory accessing. CNcomment:与内存访问相关的像素格式信息
\param[in] bSync  Synchronization. CNcomment:是否同步
...
/** 
\brief Unlocks a surface. CNcomment:解锁定surface
\attention \n
...
/** 
\brief Obtains the dimensions of a surface. CNcomment:获取surface尺寸
\attention \n
pWidth and pHeight cannot be empty concurrently.
CNcomment:pWidth与pHeight不能同时为空
\param[in]  Surface Surface handle. CNcomment:Surface句柄
\param[out]  pWidth Width output address of a surface. The value cannot be empty. CNcomment:Surface宽度输出地址，不可为空
\param[out]  pHeight Height output address of a surface. The value cannot be empty. CNcomment:Surface高度输出地址，不可为空
...
/** 
\brief Obtains the pixel format of a surface. CNcomment:获取surface像素格式
\attention \n
N/A
\param[in]  Surface Surface Surface handle. CNcomment:Surface句柄
\param[out] pPixelFormat Output address of the pixel format. The value cannot be empty. CNcomment:像素格式输出地址，不可为空
...
/** 
\brief Encapsulates the user memory into a surface. CNcomment:将用户的内存封装成surface
\attention \n
...
CNcomment:pSurInfo所有属性都必须设置正确,支持输入的像素格式为RGB格式，clut8格式，以及YUV semi-planner格式。

\param[in]  pSurInfo User memory information. The value cannot be empty. CNcomment:用户内存信息，不可为空
\param[out] pSurface Handle information. The value cannot be empty. CNcomment:句柄信息，不可为空
...
/** 
\brief Creates a memory surface. CNcomment:创建内存surface
\attention \n
...
\param[in] PixelFormat Surface pixel format. CNcomment:Surface像素格式
...
/** 
\brief Creates a child surface that shares the same memory with its parent surface. CNcomment:创建子surface，子surface与其父surface共享同一块内存
\attention \n
N/A

\param[in] Surface Parent surface handle. CNcomment:父surface句柄
\param[in] pRect Region of the child surface in the parent surface. The value cannot be empty. CNcomment:子surface在父surface中的区域，不可为空
\param[out] pSubSurface Child surface handle. The value cannot be empty. CNcomment:子surface句柄，不可为空
...
/** 
\brief Destroys a surface. CNcomment:销毁surface
\attention \n
N/A
\param[in] Surface Surface handle. CNcomment:Surface句柄
...
/**
\brief Controls whether to synchronize the drawing operations based on a surface. 允许针对surface的绘图操作是否需要自动同步。
\attention \n
...
\param[in] hSurface Surface handle, not used. CNcomment:Surface句柄，该函数内暂时没有用到
\param[in] bAutoSync Automatic sync enable. CNcomment:是否自动动步
...
CNcomment:同步surface,同步意义在于保证CPU或2D加速硬件绘制内容已经更新到surface的内存中
\attention \n
...
否则绘制将不正确。
\param[in] hSurface Surface handle, not used. CNcomment:Surface句柄，暂时无用
\param[in] mode Sync mode. For details, see the description of ::HIGO_SYNC_MODE_E. CNcomment:同步模式，参考::HIGO_SYNC_MODE_E
...
CNcomment:名字最长为16个字符，包括结尾符'\0'。

\param[in] hSurface Surface handle. CNcomment:surface句柄 
\param[in] pName String of a surface name. CNcomment:surface名字串
...
/**
\brief Obtains the name of a surface. CNcomment:获取surface的名字
\attention \n
The name contains a maximum of 16 characters including the end character '/0'.
CNcomment:名字最长为16个字符，包括结尾符'\0'。

\param[in] hSurface Surface handle. CNcomment:surface句柄
\param[in] pNameBuf Buffer for storing names. CNcomment:保存名字的buffer
\param[in] BufLen Buffer size. CNcomment:buffer 长度
...
/** 
\brief Obtains the memory type. CNcomment:获取内存类型
\attention \n
 
\param[in]  Surface Surface handle. CNcomment:Surface句柄
\param[out]  pMemType Pointer to the memory type. The value cannot be empty. CNcomment:内存类型指针，不可为空
...
/** 
\brief Queries the memory type. CNcomment:查询内存类型
\attention \n
 
\param[in]   Surface  surface
\param[out]  pOwner   Pointer to the owner type. The value cannot be empty. CNcomment:Owner类型指针，不可为空
...
5 ) YUV444情况：
	...
	u32AlignWidth_C   = u32AlignWidth_Y * 2;
	u32AlignHeight_C  = u32AlignHeight_Y;
\param[in]   pSurInfo      Pointer to the surface information. The value cannot be empty. CNcomment:surface信息指针，不可为空
\param[out]  pSurface      Pointer to a surface. The value cannot be empty. CNcomment:surface指针，不可为空
